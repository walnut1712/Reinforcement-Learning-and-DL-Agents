...
class SarsaAgent:
    """
    SARSA agent using epsilon-greedy policy.
    Updates Q-values using on-policy update rule.
    Typically learns more conservative policies around hazards.
    """
    ...
    def __init__(self, learning_rate=LEARNING_RATE, discount_factor=DISCOUNT_FACTOR,
        ...
        """
        Initialize SARSA agent with specified parameters.
        """
    ...
    def update(self, state, action, reward, next_state, next_action, done):
        ...
        Uses actual next action Q-value (on-policy), not maximum.
        This leads to more conservative behavior near hazards.
        """
        ...
        if done:
            ...
            # Bootstrap using Q-value of actual next action (on-policy)
    ...
    def train_episode(self, environment, episode_number, render=False, render_delay=0):
        """
        Execute one training episode using SARSA.
        ...
        # Select initial action (required for on-policy SARSA)
        ...
        while step_count < maximum_steps:
            ...
            # Select next action on-policy (key difference from Q-learning)
            next_action = self.select_action(next_state, self.epsilon)
            
            # Apply SARSA update with combined reward
            self.update(state, action, combined_reward, next_state, next_action, done)
            ...
            if render:
                algorithm_label = "SARSA (Intrinsic)" if self.use_intrinsic_reward else "SARSA"
    ...
    def demonstrate(self, environment, delay_milliseconds=200):
        ...
        for step_index in range(maximum_steps):
            ...
            if done:
                # Show final state
                environment.render(algorithm_name="SARSA Demo")
