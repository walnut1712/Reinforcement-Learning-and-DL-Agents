...
def parse_arguments():
    ...
    parser = argparse.ArgumentParser(description="Train rotation control agent")
    parser.add_argument(
...
def train():
    """Train the rotation control agent."""
    args = parse_arguments()
    
    # Create directories
    models_directory = "models"
    logs_directory = "logs/rotation"
    os.makedirs(models_directory, exist_ok=True)
    os.makedirs(logs_directory, exist_ok=True)
    
    # Create environment (no rendering during training)
    environment = RotationEnv(render_mode=None)
    ...
    # Save final model
    final_model_path = os.path.join(models_directory, "rotation_model")
